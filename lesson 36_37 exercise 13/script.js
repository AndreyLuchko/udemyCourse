...
// строку. Если бюджета хватает для отопления всего объема торгового центра - выводится 
...
// - вычисление общего объема торгового центра, так как цена отопления указана 
// в кубических метрах;
...
function isBudgetEnough(data) {
  let squareMall = data.shops.reduce((sum, item) => {
        return sum + item.width * item.length;
    }, 0);
    let volumeMall = squareMall * data.height;
    let payForMall = volumeMall * data.moneyPer1m3;
    if (payForMall <= data.budget) {
