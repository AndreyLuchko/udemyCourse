...
// Внутри она сначала сортирует имена по алфавиту. Затем распределяет учеников 
...
const students = ['Peter', 'Andrew', 'Ann', 'Mark', 'Josh', 'Sandra', 'Cris', 'Bernard', 'Takesi', 'Sam', 'Victor'];

function sortStudentsByGroups(arr) {
    const sortArr = arr.sort();
    const resArr = [];
    const numOfGroupe = Math.floor(arr.length / 3);
    let restStudents;

    for(let i = 0; i < numOfGroupe; i++) {
       resArr.push(sortArr.splice(0, 3)) ;
    }

    if(arr.length / 3 === 0) {
        restStudents = '-';
    } else {
        restStudents = [...sortArr].join(', ');
    }
    resArr.push(`Оставшиеся студенты: ${restStudents}`);
    return resArr;
}
